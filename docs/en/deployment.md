<!-- translated-from: docs/deployment.md sha256:e13975151d9f216dd9a93a671fb9fee2b7da78b290edeee47b0a0f3cd53f985c -->
...
| `TSMYADMIN_DOCKER_LOGIN` | `0` | With `TSMYADMIN_DOCKER_DISCOVERY=1`, `1` reads each discovered container's login from its environment (`MYSQL_ROOT_PASSWORD`, `POSTGRES_PASSWORD`, …) and lets the login screen sign in to it **in one click, with no password typed**. The password stays inside this process: it is never sent to the browser, logged or stored. **Development only** (`DISCOVERY` is refused in production, so this cannot be used there). See "Using it for development with Docker" below |
...
- **What may be connected to**: exactly the discovered containers' published `host:port` (another port on the same host, or a target that was not discovered, follows `TSMYADMIN_ALLOWED_HOSTS` as usual). From a container's environment only `MYSQL_DATABASE` / `MARIADB_DATABASE` / `POSTGRES_DB` are read, to prefill the database name; by default a password is never read, returned or logged. Enter the username and password from that project's own settings (or choose to have them read, see the next item)
- **One-click login (`TSMYADMIN_DOCKER_LOGIN=1`)**: reads the login a container was started with from its environment. MySQL / MariaDB: `MYSQL_ROOT_PASSWORD` / `MARIADB_ROOT_PASSWORD` (as root; an empty password with `MYSQL_ALLOW_EMPTY_PASSWORD` / `MARIADB_ALLOW_EMPTY_ROOT_PASSWORD`), else `MYSQL_USER` + `MYSQL_PASSWORD` (likewise `MARIADB_`). PostgreSQL: `POSTGRES_USER` (default `postgres`) + `POSTGRES_PASSWORD` (empty with `POSTGRES_HOST_AUTH_METHOD=trust`). For a container where one is found, the login screen drops the username and password fields and one press of Connect signs in. The password is kept inside this process (never returned to the browser, logged or stored), and it is only ever used for that container's own `host:port`. Note that **anyone who can reach this tool can open those databases without a password**: publish it on `127.0.0.1` only (`-p 127.0.0.1:3100:3100`) and do not use it on a shared machine or a public network. Off by default
- **When a database is missing or cannot be reached**: the login screen's "When a database is missing or cannot be reached" says why and how to fix it: a database container left off the list (stopped, port not published), why Docker cannot be read (no socket, no permission), and a listed port this process cannot open (for example one published on the host's `127.0.0.1` only, on Linux). `GET /api/servers/diagnosis` returns it. It is folded away when nothing is wrong and opens only when nothing is found or something cannot be reached
