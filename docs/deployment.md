...
| 種別 | 対応バージョン | CI で毎回検証 |
|---|---|---|
| MySQL | 8.0 / 8.4 | 8.0（最古）と 8.4（最新）で統合テスト |
| MariaDB | 10.11 (LTS) / 11 | 両方で MySQL アダプターの conformance + API 統合テスト |
| PostgreSQL | 14 / 15 / 16 / 17 | 14（最古）と 17（最新）で統合テスト。15・16 は同じカタログ面のため随時 |

- 下限は上記より古いバージョン（MySQL 5.7、PostgreSQL 13 以前、MariaDB 10.6 以前）では検証していません。5.7 は `information_schema` の構成が異なるため動作しない機能があります
- クラウドの互換サービス（Aurora、Cloud SQL、AlloyDB、TiDB など）は未検証です。プロトコル互換であれば接続はできますが、`SHOW CREATE`・`pg_depend`・`mysql.user` などカタログ依存の機能（ダンプ、アカウント管理、プロセス一覧）は差異が出ることがあります
