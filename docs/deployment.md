...
- **見つからない・つながらないとき**: ログイン画面の「接続先が見つからない・つながらないとき」が、理由と直し方を示します。一覧に出ない DB コンテナ（停止中、ポート未公開）、Docker を読めない理由（ソケットがない、権限がない）、一覧にあっても API から届かないポート（Linux でホストの `127.0.0.1` だけに公開した場合など）です。`GET /api/servers/diagnosis` が返します。問題がないときは折りたたまれ、何も見つからない・届かないときだけ開きます
