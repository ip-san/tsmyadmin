...
*English: [docs/en/operations.md](en/operations.md)*
...
tsmyadmin の永続データはセッションストア（`data/sessions.sqlite`）だけです。セッションは失っても再ログインで済みますが、`SESSION_STORE=sqlite` では保存済みクエリも同じファイルに入るため、それを残したい場合はプロセスを止めた状態でファイルごと（`-wal` / `-shm` を含む）コピーしてください（暗号化されているため、復元には同じ `SESSION_SECRET` が必要です）。バックアップ対象は接続先 DB のみです。エクスポート機能は運用バックアップの代替ではありません（一貫スナップショットではありません）。
