...
永続ストア（SQLite / Redis）は、セッションのほかに 2 種類の「保存項目」を同じ暗号化テーブルに持ちます。`savedQueries` はアカウントごと（保存済みクエリ・エクスポートのテンプレート・個人設定・セントラルカラム・列の表示変換・SQL の履歴）、`sharedItems` は DB サーバー（種別・ホスト・ポート）の全アカウント共有（ユーザーグループ・変更の追跡・共有のブックマーク）です。持ち主はどちらも HMAC で、行の中身は行ごとに結合して封をします。上限は種類ごと（アカウント 200 件、サーバー 2,000 件）で、超えると古いものから消えます。メモリストアはどちらも持たず、前者はブラウザーに保存し、後者の画面は出しません。
