...
ログインごとにセッション（= 接続プール）が作られるため、それに加えて実行中のクエリのキャンセル 1 回につき専用接続を 1 本開きます（同じ実行への同時キャンセルは 1 本を共有）。、同じ DB アカウントで保持するセッションは `SESSION_MAX_PER_IDENTITY`（既定 10）までとし、超えた分は最も古いセッションから閉じます。ログアウトせずに再ログインしたブラウザの以前のセッションも閉じます。DB 側の同時接続上限は「アカウント数 × `SESSION_MAX_PER_IDENTITY` × 4（プールあたりの接続数）」を超えないことが目安です。PostgreSQL はセッションが触れたデータベースごとにプールを持つため、さらに「× 触れるデータベース数」を見込んでください。

## SQL コンソールの行数上限について

「最大行数」は誤操作（`SELECT * FROM 巨大テーブル`）で API のメモリを使い切らないための上限で、アカウント自身に対する制限ではありません。MySQL / MariaDB は `sql_select_limit` で、PostgreSQL は派生テーブルの `LIMIT` で実現しており、文自身の `LIMIT` はそのまま尊重されます（上限を超える `LIMIT` を書いた文は派生テーブルで包み直します）。スクリプト内で `SET SESSION sql_select_limit` を実行しても、その文の後で上限を再適用します。
