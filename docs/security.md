...
- リクエスト本文の上限: `/api/session` 64 KB、SQL 実行 16 MB、インポート `IMPORT_MAX_BYTES`（64 MB）、その他の JSON 1 MB。超過は `413`（インポートはファイルが 64 MB を超えると `400 VALIDATION`、マルチパート本文が 65 MB を超えると `413`）
...
1. 新しい値（`openssl rand -hex 32`）を設定して再起動します。保存済みセッションは復号できず全て破棄され、Cookie の署名も無効になるので、全員が再ログインになります（`SESSION_STORE=memory` でも署名鍵のため手順 1 は必要です）
