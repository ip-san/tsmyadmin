...
| 消える | ECS Fargate、App Runner、Azure Container Apps、Cloud Run | `redis`（`REDIS_URL` 必須） |
...
| `TRUST_PROXY` | 前段があるなら `1`。直接公開なら `0` のまま |
| ヘルスチェック | 起動確認は `/readyz`、生存確認は `/healthz` |
| ポート | 既定 3100。`PORT` か `API_PORT` で変えられます |
...
services:
  tsmyadmin:
    image: tsmyadmin:local        # 配布イメージはありません。自分でビルドします
    environment:
      NODE_ENV: production
...
**注意**: ファイアウォールで 3100 を直接開けないでください。開いていると、リバースプロキシを迂回して `X-Forwarded-For` を自分で名乗れます。
...
| ヘルスチェック | ターゲットグループの Health check path に `/readyz` |
| メモリ | 1 GB 以上。64 MB のインポートでピークが数百 MB になります |
...
ALB も VPC も要らない分、手軽です。`PORT` が注入されるので tsmyadmin はそれに従います。TLS は App Runner が終端するので `TRUST_PROXY=1`。ディスクは消えるので Redis が必要です。RDS / ElastiCache に届かせるには VPC コネクタを設定してください。
...
| レプリカ | 最小 1。0 まで縮めるとアクセスのたびにコールドスタートします |
| シークレット | Container Apps のシークレットを環境変数に割り当てます |

### App Service for Containers

動きますが、`WEBSITES_PORT` に 3100 を設定する必要があります。
...
> ここに挙げた前段の形は、`apps/api/src/platform-conformance.test.ts` で全部テストしています。各形について「利用者を識別できるか」「1 人をレート制限しても他人が巻き込まれないか」「ユーザー名を変えながらの総当たりが止まるか」を確認しています。**新しい置き場所を足すときは、この表と同じ行をテストにも足してください。**

## 確認済みと未確認

**確認済み**: 本番イメージを `SESSION_STORE=redis` と `TRUST_PROXY` を設定して起動し、ログイン・セッションの Redis 書き込み・転送ヘッダーからのクライアント IP 取得までをローカルで確認しています。

**未確認**: AWS・Azure の実アカウントでの動作。上の各表は各サービスのドキュメントに基づくもので、実機では確かめていません。最初のデプロイでは、[cloudflare.md](cloudflare.md) の「動いているか確認する」と同じ 3 点（DB / Redis へ到達できるか、クライアント IP が利用者ごとに違うか、停止時に猶予があるか）を必ず確認してください。
