...
- **DB 抽象**: `DatabaseAdapter` を `mysql2` / `pg` の上に薄く実装。ORM 不使用（Prisma は不採用）。閲覧・CRUD・SQL 実行・DDL・エクスポート（`showCreateTable`/`iterateRows`/`exporter`）・インポート（`insertRows`）・アカウント（`listUsers`/`showGrants`/`canManageAccount`/`users` ビルダー）・サーバー情報（`serverInfo`/`listVariables`/`listStatus`/`listProcesses`/`killProcess`/`serverCatalog`）を両方言で同じ契約に揃える
...
- ユニット/API/Web テスト定義: <!-- stat:unit-tests -->494<!-- /stat --> 件
- Adapter conformance: <!-- stat:conformance -->146<!-- /stat --> 件 × 2 方言
- E2E: <!-- stat:e2e -->86<!-- /stat --> 件
- API ルート: <!-- stat:routes -->52<!-- /stat -->
