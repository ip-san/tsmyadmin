...
describe('SecondFactorPage', () => {
  it('says why it is unavailable instead of offering an enrolment that cannot be kept', async () => {
    ...
    expect(screen.queryByRole('button', { name: '認証アプリで登録する' })).not.toBeInTheDocument()
  })
  ...
  it('can start over once a secret has been shown', async () => {
    ...
    renderPage('none')
    await userEvent.click(await screen.findByRole('button', { name: '認証アプリで登録する' }))
    ...
    expect(await screen.findByRole('button', { name: '認証アプリで登録する' })).toBeInTheDocument()
