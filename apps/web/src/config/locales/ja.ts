...
export const ja = {
  ...
  queryBuilder: {
    ...
    tables: '対象のテーブル',
    tablesHint: '最初に選んだテーブルを起点に、ほかのテーブルを LEFT JOIN します。',
    ...
    desc: '降順',
    addColumn: 'カラムを追加',
    outputLabel: (n: number) => `出力 ${n} 行目の`,
    removeOutput: (label: string) => `${label}カラムを外す`,
    criteria: '条件',
    group: (n: number) => `条件グループ ${n}（すべて満たす）`,
    or: 'または',
    conditionLabel: (group: number, n: number) => `グループ ${group} の条件 ${n} の`,
    operator: '演算子',
    value: '値',
    addCondition: 'AND 条件を追加',
    addFirstCondition: '条件を追加',
    addGroup: 'OR 条件グループを追加',
    removeCondition: (label: string) => `${label}条件を削除`,
    remove: '削除',
    build: 'SQL を作成',
    building: '作成中…',
    result: '作成した SQL',
