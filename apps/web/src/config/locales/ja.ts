...
export const ja = {
  ...
  databaseOps: {
    ...
    renameHintMysql:
      'MySQL には名前変更の命令がないため、新しいデータベースを作ってテーブルを移します。元のデータベースは削除せずに残すので、中身が空になったことを確認してから削除してください。ビュー・ルーチン・トリガー・イベントがあるデータベースは変更できません。',
    ...
    renameWarningMysql:
      '元のデータベースは削除されずに残ります（あなたのアカウントから見えないルーチンやイベント、この確認の後に作られたテーブルを巻き込まないため）。中身を確認してから削除してください。データベースに付けた権限（GRANT）は新しい名前に引き継がれません。',
