...
describe('SqliteSessionStore', () => {
  ...
  it('drops a saved-query row that will not open, instead of carrying it forever', async () => {
    ...
    try {
      await store.savedQueries.save(config, 'sql', 'keep', 'SELECT 1')
      await store.savedQueries.save(config, 'sql', 'break', 'SELECT 2')
      const ids = await store.savedQueries.list(config, 'sql')
      ...
      expect((await store.savedQueries.list(config, 'sql')).map((q) => q.name)).toEqual(['keep'])
  ...
  it('rotates the secret on a file written before saved queries existed', async () => {
    ...
    try {
      ...
      expect(await store.savedQueries.list(config, 'sql')).toEqual([])
      expect(await store.savedQueries.save(config, 'sql', 'daily', 'SELECT 1')).toHaveLength(1)
    } finally {
      await store.closeAll()
    }
  })
  ...
  it('carries sessions and saved queries across the row-binding upgrade', async () => {
    ...
    try {
      ...
      await before.savedQueries.save(config, 'sql', 'daily', 'SELECT 1')
    } finally {
      await before.closeAll()
    }
    ...
    try {
      ...
      expect(await after.savedQueries.list(config, 'sql')).toMatchObject([{ name: 'daily', body: 'SELECT 1' }])
  ...
  it('re-runs the binding migration over already-bound rows without losing them', async () => {
    ...
    try {
      ...
      expect(await again.savedQueries.list(config, 'sql')).toMatchObject([{ name: 'daily' }])
    } finally {
  ...
  it('leaves a rolled-back image a readable file, not a broken one', async () => {
    ...
    try {
      await store.create(config)
      await store.savedQueries.save(config, 'sql', 'daily', 'SELECT 1')
    } finally {
      await store.closeAll()
    }
  ...
  it('keeps saved queries per account, sealed, and capped', async () => {
    ...
    try {
      const saved = await store.savedQueries.save(config, 'sql', 'recent users', "SELECT * FROM users WHERE note = 'x'")
      ...
      expect(await store.savedQueries.save(config, 'sql', 'recent users', 'SELECT 2')).toHaveLength(1)
      expect((await store.savedQueries.list(config, 'sql'))[0]?.body).toBe('SELECT 2')

      // Another account sees nothing of it, and cannot delete it by id.
      expect(await store.savedQueries.list(other, 'sql')).toEqual([])
      await store.savedQueries.remove(other, 'sql', saved[0]?.id ?? '')
      expect(await store.savedQueries.list(config, 'sql')).toHaveLength(1)
      ...
      await store.savedQueries.remove(config, 'sql', saved[0]?.id ?? '')
      expect(await store.savedQueries.list(config, 'sql')).toEqual([])
    } finally {
      await store.closeAll()
    }
