...
for (const t of TARGETS) {
  test.describe(`accessibility (axe-core, ${t.dialect})`, () => {
    ...
    test('server status, processes and users screens', async ({ page }) => {
      ...
      await scan(page)
      await page.getByRole('table', { name: '通信量と接続' }).waitFor()
      await scan(page)
      await page.goto('/processes')
      await page.getByRole('table', { name: 'プロセス一覧' }).waitFor()
      await scan(page)
      await page.goto('/monitor')
      await page.getByRole('heading', { name: '記録された文' }).waitFor()
      await page.getByRole('img').first().waitFor({ timeout: 15_000 })
      await scan(page)
      await page.goto('/advisor')
      await page.getByRole('heading', { name: 'アドバイザー', level: 2 }).waitFor()
      await scan(page)
      await page.goto('/users')
