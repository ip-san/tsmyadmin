...
## adapter: `replicationInfo` の `role` は「権限がなくて null」と「本当に空」を区別せず、権限が弱いアカウントには常に `standalone` と嘘をつく（f85ae15、両方言で実機確認済み）

- 場所: `packages/adapter/src/mysql/server.ts` の `mysqlReplicationInfo`（`reading = (source?.length ?? 0) > 0`）と `packages/adapter/src/postgres/server.ts` の `pgReplicationInfo`（同じ式）。`source`/`replicas` が「権限がなくて `null`」でも「確認できて 0 件」でも同じ `?? 0` に潰れるため、`role` の判定式（`reading && sending ? 'relay' : reading ? 'replica' : sending ? 'primary' : 'standalone'`）はどちらのケースも `standalone` にする。`ReplicationInfoSchema.role`（`packages/shared/src/schemas/server.ts`）にも `'unknown'` 相当の値がなく、型レベルでも区別できない。
- 実機確認（MySQL 8.4、`SELECT` 権限のみで `REPLICATION CLIENT`/`REPLICATION SLAVE` を持たないアカウントを作り、`MysqlAdapter#replicationInfo()` を直接呼んだ）: `{ role: 'standalone', source: null, replicas: null, logs: null }`。実際にこのサーバーがレプリカ/プライマリかどうかは（`source`/`replicas` が読めていないので）本来わからないはずなのに、`role` は確信ありげに `standalone` を返す。PostgreSQL 側は `pg_stat_wal_receiver`/`pg_stat_replication` 自体は無権限でも読める（列がNULLになるだけで行は返る）設計なので実害は薄いが、`pg_ls_waldir()` 権限がない場合 `logs: null` になり、`role` の計算ロジック自体はMySQLと同じ潰れ方をする（実際に再現するには pg_stat_* への SELECT 自体を revoke するような非典型構成が要る）。
- UI（`apps/web/src/features/server/ReplicationPage.tsx`）は矛盾した見え方になる: 上部の役割バッジは自信ありげに「スタンドアロン」（`tone: 'neutral'`）と表示する一方、その下の source/replicas/logs のセクションは同じデータに対して「権限がないため確認できません」の `Notice` を出す——同じ画面の中で「わからない」と「たしかにスタンドアロン」が両方主張される。
- テストはこれを気づけない設計になっている: `packages/adapter/src/test/conformance.ts` の `describe('replicationInfo')` は権限の弱いアカウントに対して `expect(limited.role).toBe('standalone')` を**期待値として固定**しており、この挙動をバグではなく仕様として検査している。次にこのテストを見たら「本当にこれが正しい仕様か」を疑うこと（少なくとも今回のレビューでは Warning として指摘済み・未修正）。
- 修正案（提案のみ）: `role` に `'unknown'` を追加し、`source === null || replicas === null` のときは `reading`/`sending` を確定させず `'unknown'` を返す。あるいは最低限、UI 側で「source と replicas がともに null なら役割バッジも `Notice` にする」等の対症療法。
