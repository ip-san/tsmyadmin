...
### 15. 複合キーの区切りなし連結（diff を読む。grep では精度が出ない）

grep での自動検出は試したところ機能しなかった: 過去に再発した本物のバグ（`` `central.${scope}.${database}.${schema ?? ''}` ``）は区切りが `.` なので単純な「隣接パターン」では引っかからず、逆に隣接だけを緩く広げると `` `${session.dialect}.${session.host}.${session.port}` ``（この codebase で意図的に多用されているスコープキー）まで含めて60箇所以上ヒットする。**このため grep ではなく diff を目で読む。**

**判定:** 変更差分に、**テーブル名・スキーマ名・データベース名・カラム名（ユーザーが選べる識別子）** を2つ以上テンプレートリテラルで連結して、ローカルストレージのキーや React の `key` を作るコードがあれば、区切り文字なし、または `.` だけの区切りになっていないか確認する。識別子自体に区切り文字と同じ文字（`.` など）が入ると衝突する。見つけたら既存パターンの `JSON.stringify([a, b, ...])` に揃える（`TableShortcuts.tsx` で一度修正済みなのに `central-columns.ts` / `column-transforms.ts` で再発した既往症）。`session.dialect` / `host` / `port` を連結する既存の `scope` パターンは対象外（値が識別子ではなく、意図的に広く使われている）。
...
| 15 | 複合キーの区切りなし連結 | OK / 要確認 | diff で見つかった箇所 |
