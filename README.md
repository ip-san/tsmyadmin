...
- 既定で読むのはコンテナの一覧とデータベース名だけで、**パスワードは読みも保存もしません**。`TSMYADMIN_DOCKER_LOGIN=1` を明示したときだけ、コンテナの環境変数の資格情報を API のプロセスの中に読み、パスワードなしの 1 クリックで入れます（ブラウザには返さず、ログにも保存にも出しません。届く人は誰でも入れるので、`127.0.0.1` にだけ公開してください。`docs/deployment.md`）
