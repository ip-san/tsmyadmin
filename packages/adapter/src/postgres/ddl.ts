...
function columnDef(c: ColumnSpec): string {
  ...
  if (c.collation) parts.push(`COLLATE ${id(c.collation)}`)
  if (c.generated) {
    // Computed by the server: no default or identity of its own. VIRTUAL needs PostgreSQL 18.
...
/**
 * What changing a column's generation takes: a new expression (PostgreSQL 17), or dropping it to keep the values
 * as an ordinary column. Making an existing column generated, or switching STORED / VIRTUAL, is not something
 * PostgreSQL can do in place.
 */
function generatedChange(
  t: string,
  col: string,
  /** undefined when the current definition is not known: the expression is then written as given. */
  before: ColumnSpec['generated'] | undefined,
  after: ColumnSpec['generated']
): string[] {
  if (before === undefined)
    return after ? [`ALTER TABLE ${t} ALTER COLUMN ${col} SET EXPRESSION AS (${after.expression})`] : []
  if (!before && !after) return []
  if (before && !after) return [`ALTER TABLE ${t} ALTER COLUMN ${col} DROP EXPRESSION`]
  if (!before || !after || before.stored !== after.stored) {
    throw new AdapterError(
      'UNSUPPORTED',
      'PostgreSQL cannot make an existing column generated, or change how it is kept'
    )
  }
  if (before.expression === after.expression) return []
  return [`ALTER TABLE ${t} ALTER COLUMN ${col} SET EXPRESSION AS (${after.expression})`]
}
  ...
  build(ns: Namespace, op: DdlOp): string[] {
    ...
    switch (op.op) {
      ...
      case 'addColumn':
        ...
        return [
          `ALTER TABLE ${t} ADD COLUMN ${columnDef(op.column)}`,
          ...commentSql(t, op.column),
          ...columnKeySql('postgres', ns, op),
        ]
      case 'modifyColumn': {
        const c = op.column
        const prev = op.previous
        if (op.first || op.after) throw new AdapterError('UNSUPPORTED', 'PostgreSQL cannot reorder columns')
        const out: string[] = []
        if (op.name !== c.name) out.push(`ALTER TABLE ${t} RENAME COLUMN ${id(op.name)} TO ${id(c.name)}`)
        const col = id(c.name)
        const generated = generatedChange(t, col, prev === undefined ? undefined : prev.generated, c.generated)
        ...
        if (!c.autoIncrement && !c.generated && (prev === undefined || defaultSql(prev) !== defaultSql(c))) {
          const def = defaultSql(c)
