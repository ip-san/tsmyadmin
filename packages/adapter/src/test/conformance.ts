    ...
    describe('listDependencies', () => {
      it('reports what a view reads from the catalog, or null where the server has none (MariaDB)', async () => {
        ...
        try {
          ...
          await execOk(`DROP VIEW ${v2}; DROP VIEW ${v1}; DROP TABLE ${t}`)
        }
      })

      it.skipIf(dialect !== 'postgres')('records what a SQL-standard-body routine reads (PostgreSQL)', async () => {
        const t = `${scratch}_dep_t`
        ...
        try {
          const deps = (await db.listDependencies(ns)) ?? []
          expect(deps.find((d) => d.kind === 'routine' && d.name === f)?.dependsOn).toContainEqual({
            kind: 'view',
            name: v,
          })
        } finally {
          await execOk(`DROP FUNCTION ${f}(); DROP VIEW ${v}; DROP TABLE ${t}`)
        }
      })
    })
